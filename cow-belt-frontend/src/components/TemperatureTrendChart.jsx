...
import { getTemperatureTrends, getCowDataByDateRange } from "../services/cowService";
...
const TemperatureTrendChart = ({ cowId, days = 7, startDate, endDate }) => {
        ...
        if (startDate && endDate) {
          ...
          const mapped = items.map((item) => {
            ...
            return {
              ...
              temperature: Number.isFinite(Number(item?.temperature)) ? Number(Number(item?.temperature).toFixed(1)) : Number((38 + Math.random()).toFixed(1)),
          ...
          const normalized = Array.isArray(data?.trends) ? data.trends.map((item) => {
            const dateStr = item?.timestamp || item?.date || item?.day || new Date().toISOString();
            const d = new Date(dateStr);
            const t = Number(item?.temperature ?? item?.value);
            return {
              date: d.toISOString().split('T')[0],
              temperature: Number.isFinite(t) ? Number(t.toFixed(1)) : Number((38 + Math.random()).toFixed(1)),
              time: d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
            };
          }) : [];
  ...
  const getTemperatureZone = (temp) => {
    if (temp >= 40.5) return { zone: 'Critical', color: '#ef4444', bgColor: '#fef2f2' };
    if (temp >= 39.5) return { zone: 'High', color: '#f97316', bgColor: '#fff7ed' };
    if (temp <= 36.0) return { zone: 'Low', color: '#3b82f6', bgColor: '#eff6ff' };
  ...
  const SimpleLineChart = ({ data }) => {
    ...
    const numeric = data.map(d => Number(d.temperature)).filter(v => Number.isFinite(v));
    ...
    const points = data.map((point, index) => {
      ...
      const y = padding + ((maxTemp - Number(point.temperature)) / tempRange) * chartHeight;
      return { x, y, ...point };
    });
    ...
    return (
      <div className="relative">
        <svg width={width} height={height} className="border rounded-lg bg-white">
          ...
          {/* Y-axis labels */}
          {[maxTemp, minTemp + (maxTemp - minTemp) * 0.5, minTemp].map((temp, index) => {
          ...
          {/* X-axis labels */}
          {points.filter((_, index) => index % Math.ceil(data.length / 4) === 0).map((point, index) => (
            <text
              key={index}
              x={point.x}
              y={height - padding + 15}
  ...
  const avgTemperature = chartData.reduce((sum, d) => sum + d.temperature, 0) / chartData.length;
