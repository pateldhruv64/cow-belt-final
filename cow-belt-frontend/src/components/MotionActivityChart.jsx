...
import { getMotionAnalysis, getCowDataByDateRange } from "../services/cowService";
...
const MotionActivityChart = ({ cowId, days = 7, startDate, endDate }) => {
    ...
    const fetchMotionData = async () => {
      try {
        ...
        if (startDate && endDate) {
          const rangeRes = await getCowDataByDateRange(startDate, endDate, cowId);
          const items = Array.isArray(rangeRes?.data) ? rangeRes.data : [];
          const mapped = items.map((item) => {
            const dateStr = item?.timestamp || item?.createdAt || item?.date || new Date().toISOString();
            const d = new Date(dateStr);
            const motionValue = Number((item?.motionChange ?? item?.motion ?? 40 + Math.random() * 20).toFixed(1));
            return {
          ...
          const normalized = Array.isArray(data?.analysis) ? data.analysis.map((item) => {
            const dateStr = item?.timestamp || item?.date || item?.day || new Date().toISOString();
            const d = new Date(dateStr);
            const raw = item?.motion ?? item?.motionChange ?? item?.value ?? item?.activity;
  ...
  const chartData = (Array.isArray(motionData) && motionData.length > 0) ? motionData : generateMockData();

  // Calculate activity levels
  const getActivityLevel = (motion) => {
    if (motion >= 100) return { level: 'Very Active', color: '#ef4444', bgColor: '#fef2f2', icon: '🏃‍♂️' };
    if (motion >= 60) return { level: 'Active', color: '#f97316', bgColor: '#fff7ed', icon: '🚶‍♂️' };
    if (motion >= 20) return { level: 'Normal', color: '#10b981', bgColor: '#f0fdf4', icon: '🐄' };
    if (motion >= 10) return { level: 'Low', color: '#3b82f6', bgColor: '#eff6ff', icon: '😴' };
    return { level: 'Inactive', color: '#6b7280', bgColor: '#f9fafb', icon: '💤' };
  };
  ...
  const SimpleBarChart = ({ data }) => {
    ...
    const numeric = data.map(d => Number(d.motion)).filter(v => Number.isFinite(v));
      ...
      <div className="relative">
        <svg width={width} height={height} className="border rounded-lg bg-white">
          ...
          {data.map((point, index) => {
            ...
            const denom = (maxMotion && Number.isFinite(maxMotion)) ? maxMotion : 1;
            const barHeight = (Number.isFinite(safeMotion) ? (safeMotion / denom) : 0) * chartHeight;
            const y = padding + chartHeight - barHeight;
            const activityLevel = getActivityLevel(Number.isFinite(safeMotion) ? safeMotion : 0);
          ...
          {[maxMotion, maxMotion * 0.5, 0].map((motion, index) => {
            const y = padding + ((maxMotion - motion) / motionRange) * chartHeight;
            return (
              <text
                key={index}
                x={padding - 10}
          ...
          {/* X-axis labels */}
          {data.filter((_, index) => index % Math.ceil((data.length || 1) / 5) === 0).map((point, index) => (
            <text
              key={index}
  ...
  const avgMotion = chartData.reduce((sum, d) => sum + d.motion, 0) / chartData.length;
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-green-600">{latestMotion?.toFixed(1)}</div>
          <div className="text-sm text-green-600">Current</div>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-blue-600">{avgMotion.toFixed(1)}</div>
          <div className="text-sm text-blue-600">Average</div>
        </div>
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-orange-600">{maxMotion.toFixed(1)}</div>
          <div className="text-sm text-orange-600">Peak</div>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-center">
          <div className="text-2xl font-bold text-gray-600">{minMotion.toFixed(1)}</div>
      ...
      <div className="mt-6">
        <h4 className="text-lg font-semibold text-gray-800 mb-3">Activity Distribution</h4>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {Object.entries(activityDistribution).map(([level, count]) => {
            const activityLevel = getActivityLevel(level === 'Very Active' ? 100 : level === 'Active' ? 80 : level === 'Normal' ? 40 : level === 'Low' ? 15 : 5);
            ...
            return (
              ...
              >
                ...
                <div className="text-lg font-bold" style={{ color: activityLevel.color }}>
                  {count}
                </div>
        ...
        <div className="mt-6 p-4 rounded-lg" style={{ 
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <span className="text-lg">{getActivityLevel(latestMotion).icon}</span>
              <span className="font-medium text-gray-800">Current Activity:</span>
              <span 
                className="font-bold"
